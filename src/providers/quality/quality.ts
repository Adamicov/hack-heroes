...
/*
  Generated class for the QualityProvider provider.
...
export class QualityProvider {
  ...
  constructor(public http: HttpClient) {
    console.log('Hello QualityProvider Provider');
  }



  partQuality = function(pollution){
    let q;
    let name = pollution.paramFormula;
    let val = pollution.value;
    if(name=='PM10'){
      if(val>0&&val<=20){
        q=0;
  ...
  AirQuality = function(station){
    let Q = [];
    let maxq = 0;
    for(let i=0; i < station.pollutions.length; i++){
      Q.push(this.partQuality(station.pollutions[i]))
